...
static void LCD_GPIOInit(void){
    ...
    wait++;                                     // wait for port activation
    GPIODIR_D |= 0x04;                      // make PD2 an output
    GPIOAFSEL_D &= ~0x04;                         // disable alternate functions
    GPIODEN_D |= 0x04;                       // enable digital I/O on PD2
    ...
    GPIOAFSEL_K &= ~0x20;                      // disable alternate functions
    ...
    GPIOAFSEL_M &= ~0x80;                        // disable alternate functions
    ...
    GPIOAFSEL_N &= ~0x30;                       // disable alternate functions
    ...
    GPIOAFSEL_P &= ~0x1B;                     // disable alternate functions
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_Q |= 0x0D;                           // make PQ0, PQ2-3 outputs
    GPIOAFSEL_Q &= ~0x0D;                      // disable alternate functions
    GPIODEN_Q |= 0x0D;                         // enable digital I/O on PQ0, PQ2-3
}
...
void LCD_PrintFloat(double num){
    // TODO: You code here.
  char buffer[20];
  snprintf(buffer, sizeof(buffer), "%4.2f", num);
...
void LCD_DrawFilledRect(unsigned short x, unsigned short y,
    ...
    for (i = 0; i < height; i++) {
        ...
        // Set the Y address of the display \.
        LCD_WriteCommand(SSD2119_Y_RAM_ADDR_REG);
        LCD_WriteData(y + i);
